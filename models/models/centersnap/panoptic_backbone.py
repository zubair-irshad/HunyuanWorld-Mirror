...
MODEL_POSE_HEAD_COMMON_STRIDE = 2
...
class PoseFPNHead(nn.Module):
  ...
  def __init__(self, input_shape, num_classes, model_norm='BN', num_filters_scale=4):
    ...
    norm = MODEL_SEM_SEG_HEAD_NORM
            ...
            # 🔹 This is equivalent to SemSegFPNHead's final upsample
    self.bilinear_upsample = nn.Upsample(scale_factor=self.common_stride, mode="bilinear", align_corners=False)
  ...
  def forward(self, features, targets=None):
    ...
    # x = self.bilinear_upsample(x)  # upsample to input res
    # print("x after upsample", x.shape)  # Debug print
    return x
